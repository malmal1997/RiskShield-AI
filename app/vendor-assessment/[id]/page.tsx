...
function VendorAssessmentComponent() {
  ...
  useEffect(() => {
    async function loadAssessment() {
      try {
        ...
        const aiParam = searchParams?.get("ai") // Null check for searchParams
        ...
        if (assessmentId && assessmentId.startsWith("internal-")) { // Ensure assessmentId exists
          // Get delegation info
        ...
        if (!assessmentId) { // Ensure assessmentId exists for external assessments
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const isInternalDelegation = assessmentId && assessmentId.startsWith("internal-") // Ensure assessmentId exists
      ...
      if (isInternalDelegation) {
        ...
        if (!assessmentId) { // Ensure assessmentId exists for external submission
          throw new Error("Assessment ID is missing for external submission.");
        }
    ...
    } catch (error: any) // Explicitly type error
    {
      ...
      console.error("Error details:", {
        ...
        stack: error.stack,
        assessmentId,
        isInternalDelegation: assessmentId && assessmentId.startsWith("internal-"), // Ensure assessmentId exists
      })
          ...
          <div className="mx-auto max-w-4xl px-4 sm:px-6 lg:px-8">
            ...
            {isAiPoweredAssessment && currentStep === 1 && showReviewStep && analysisResults && (
              <Card>
                ...
                </CardHeader<dyad-problem-report summary="3 problems">
<problem file="app/vendor-assessment/[id]/page.tsx" line="466" column="24" code="18047">'params' is possibly 'null'.</problem>
<problem file="app/vendor-assessment/[id]/page.tsx" line="467" column="17" code="18047">'searchParams' is possibly 'null'.</problem>
<problem file="app/vendor-assessment/[id]/page.tsx" line="507" column="25" code="18047">'searchParams' is possibly 'null'.</problem>
</dyad-problem-report><think>**Fixing TypeScript Errors**

I'm currently addressing TypeScript errors in `app/vendor-assessment/[id]/page.tsx`. Specifically, `params` and `searchParams` are potentially null. My plan involves handling each case where these values could be absent. I'm focusing on ensuring robust type safety and preventing runtime errors by incorporating null checks and type guards throughout the code.


**Resolving Parameter Issues**

I'm now fixing the remaining TypeScript errors in `app/vendor-assessment/[id]/page.tsx`. Specifically, I'm addressing the potential `null` values of `params` and `searchParams`. I'll first check if `params` is present before accessing `params.id` in `loadAssessment` function to prevent runtime errors. For `searchParams`, I'll include null checks to safely access `"token"` and `"ai"` values to maintain type safety and prevent crashes.


**Tackling Null Checks**

I've outlined three potential TypeScript errors related to `params` and `searchParams` possibly being null in `app/vendor-assessment/[id]/page.tsx`. To address these, I'm implementing null checks before accessing `params.id` in the `loadAssessment` function, returning early if `params` is absent. For `searchParams`, I'll include null checks before calling `.get("token")` and `.get("ai")`. This strategy guarantees robust type safety and prevents potential runtime errors caused by unexpected null values. Now, I'm ready to apply these fixes to the code.


</think>You're right, those `possibly 'null'` errors are common with Next.js router hooks during initial renders or when parameters aren't present. I'll add concise null checks for `params` and `searchParams` to resolve these.

Here's the updated `app/vendor-assessment/[id]/page.tsx` file:

<dyad-write path="app/vendor-assessment/[id]/page.tsx" description="Adding null checks for 'params' and 'searchParams' before accessing their properties.">
"use client"
...
function VendorAssessmentComponent() {
  ...
  // Add null checks for params and searchParams
  const assessmentId = (params?.id as string) || ''; // Default to empty string if null
  const token = searchParams?.get("token");
  ...
  useEffect(() => {
    async function loadAssessment() {
      try {
        ...
        // Ensure assessmentId is valid before proceeding
        if (!assessmentId) {
          setError("Assessment ID is missing.");
          setIsValidToken(false); // Treat as invalid link if ID is missing
          setIsLoading(false);
          return;
        }
        ...
        // For internal delegations - redirect to appropriate internal assessment page
        if (assessmentId.startsWith("internal-")) { // assessmentId is guaranteed to be a string here
          // Get delegation info
          const delegationKey = `delegation-${assessmentId}`
          const delegationInfo = localStorage.getItem(delegationKey)
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // Check if this is an internal delegation (ID starts with "internal-")
      const isInternalDelegation = assessmentId.startsWith("internal-") // assessmentId is guaranteed to be a string here
      console.log("🏢 Is Internal Delegation:", isInternalDelegation)
      ...
      if (isInternalDelegation) {
        ...
        console.log("🔄 Processing external vendor assessment submission...")
        // Submit to database for external vendor assessments
        await submitAssessmentResponse(assessmentId, vendorInfo, answers) // assessmentId is guaranteed to be a string here
        console.log("✅ External assessment submitted to database")
      }
    ...
    } catch (error: any) // Explicitly type error
    {
      ...
      console.error("Error details:", {
        ...
        stack: error.stack,
        assessmentId,
        isInternalDelegation: assessmentId.startsWith("internal-"), // assessmentId is guaranteed to be a string here
      })
      alert("Failed to submit assessment. Please try again.")
    }
  }
