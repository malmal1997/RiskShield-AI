  ...
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="max-h-[95vh] flex flex-col">
        ...
        <ScrollArea className="flex-1"> {/* Removed overflow-x-auto from here */}
          <div className="py-4 space-y-6 w-max overflow-x-auto"> {/* Added w-max and overflow-x-auto here */}
            {/* Report Summary */}
            ...
            {vendorInfo && (
              <div className="p-4 bg-gray-50 border border-gray-200 rounded-lg">
                ...
                <p className="text-sm text-gray-700 whitespace-pre-wrap"><strong>Company Name:</strong> {vendorInfo.companyName}</p>
                <p className="text-sm text-gray-700 whitespace-pre-wrap"><strong>Contact Person:</strong> {vendorInfo.contactName}</p>
                <p className="text-sm text-gray-700 flex items-center whitespace-pre-wrap">
                ...
                {vendorInfo.industry && <p className="text-sm text-gray-700 whitespace-pre-wrap"><strong>Industry:</strong> {vendorInfo.industry}</p>}
                {vendorInfo.employeeCount && <p className="text-sm text-gray-700 whitespace-pre-wrap"><strong>Employees:</strong> {vendorInfo.employeeCount}</p>}
                {vendorInfo.description && <p className="text-sm text-gray-700 whitespace-pre-wrap"><strong>Description:</strong> {vendorInfo.description}</p>}
              </div>
            )}
